...
public class TimeTableService {
	...
	public int createPeriods(TimeTableDataTransferObject timetable) {
        ...
        try {
            ...
            if (checkIfExistingTime(timetable)) {
                ...
                clsprds.setSubjectid(db.where("subjectname =?", timetable.getSubjectname()).results(Subjects.class)
                        .get(0).getId());
    ...
    private boolean checkIfExistingTime(TimeTableDataTransferObject timetable) {
        ...
        List<ShowPeriods> from = db.sql(
                "select * from classroom_periods where periodfrom = ? and classroomweekdayid = ?", tFrom,
                timetable.getDayid()).results(ShowPeriods.class);
        ...
        if (from.isEmpty()) {
            if (to.isEmpty()) {
                List<ShowPeriods> existingFromTime = db.sql(
                        "select * from classroom_periods where classroomweekdayid =?", timetable.getDayid()).results(
                        ShowPeriods.class);
                List<ShowPeriods> existingToTime = db.sql(
                        "select * from classroom_periods where classroomweekdayid =?", timetable.getDayid()).results(
                        ShowPeriods.class);
                return isBetween(tFrom, tTo, existingFromTime, existingToTime);
            }
		}
        return flag;
	}
    ...
    private boolean isBetween(LocalTime tFrom, LocalTime tTo, List<ShowPeriods> existingFromTime,
            List<ShowPeriods> existingToTime) {
        ClassRoomPeriods clsperiods = new ClassRoomPeriods();
		boolean flag = false;
        for (int i = 0; i < existingFromTime.size(); i++) {
            clsperiods.setPeriodfrom(LocalTime.of(existingFromTime.get(i).getPeriodfrom().getHours(), existingFromTime
                    .get(i).getPeriodfrom().getMinutes()));
    ...
    public List<TableView> getClassPeriods(Long id) {
		db = retrive.getDatabase(1);
        return db.sql("select wd.day,sbj.subjectname, cp.periodfrom, cp.periodto, cp.dateofassigning"
                + " from classroom_periods cp join subjects sbj on sbj.id = cp.subjectid"
                + " join weekdays wd on wd.id = cp.classroomweekdayid where classroomid=?",id).results(TableView.class);
	}

...
public class ParentController {
	...
	public List<ParentDTO> listParentChildrens(@PathVariable("tenantId") long tenantId,StudentClassroom studentClassroom) {
		long parentid=studentClassroom.getId();
		return service.listParentChildrens(parentid, tenantId);
	}
	@PreAuthorize("hasAnyRole('ROLE_PARENT')")
	@RequestMapping(value = "/{studentid}/marks", method = RequestMethod.POST, produces = "application/json")
	public List<TestTransferObject> studentMarks(@PathVariable("studentid") long studentid,
			@PathVariable("tenantId") long tenantId, ParentDTO parentDTO) {
		long classid=parentDTO.getClassid();
		long gradeid = parentDTO.getGradeid();
		return service.studentMarks(studentid, gradeid,classid,tenantId);
	}
	@PreAuthorize("hasAnyRole('ROLE_PARENT')")
	...
	public List<AttendanceDTO> attendanceReport(@PathVariable("tenantId") long tenantId,StudentClassroom studentClassroom) {
